...
CONTAINS             
   ...
   SUBROUTINE usr_def_zgr( ld_zco  , ld_zps  , ld_sco  , ld_isfcav,    &   ! type of vertical coordinate
      ...
      REAL(wp) ::   zmaxdpt, dz       ! maximum depth
      ...
      IF(lwp) THEN            ! Parameter print
         WRITE(numout,*)
         WRITE(numout,*) '     zgr_z32L   : Reference vertical z-coordinates '
         WRITE(numout,*) '     ~~~~~~~'
         WRITE(numout,*) '       C1D case : L32 function with the following coefficients :'
      ...
      zht(:,:) = rn_bathy
      !
      dz = rn_bathy / REAL( jpk, wp )
      DO jk = 1, jpk          ! depth at T and W-points with regular spacing
         zw = REAL( jk , wp )
         zt = REAL( jk , wp ) + 0.5_wp
         pdepw_1d(jk) = (zw - 1.0_wp) * dz
         pdept_1d(jk) = (zt - 1.0_wp) * dz
      END DO
      !
